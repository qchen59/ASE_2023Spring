...
def generateBothTablesForDataset(dataset_path: Path, retry_attempt: int = 0):
    clustered_result = clusterDataset(dataset_path)
    table1 = getTable1(clustered_result)
    range_result = createRanges(clustered_result)
    table2 = getTable2(range_result)

    # check if sway1=sway2
    counts = Counter(table2.loc['sway1 to sway2'].to_list())
    different = True if counts['='] == len(counts) else False

    if retry_attempt == RE_RUNS or different:
        return [table1, table2]

    # recursive call
    generateBothTablesForDataset(dataset_path, retry_attempt+1)
...
def main():
    ...
    for dataset in datasets[2:4]:
        table1, table2 = generateBothTablesForDataset(dataset)
        print(f'{dataset:^20}')
        print(table1)
        print(table2)
