...
def RANGE(at: int, txt: str, lo: int, hi: int = None) -> dict:
...
def bin(col: sym.Sym | Num, x: int) -> str | sym.Sym | int:
...
def extend(range: dict, n: int, s: str) -> None:
...
def value(has: dict, nB: int = 1, nR: int = 1, sGoal: bool = True) -> float:
